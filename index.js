$(document).ready(() => {
    ...
    ScrapeButton.click(() => {
      ...
      if (!val || val === "" || !validateUrl(val))
        return SendErr("Please specify correct URL", "Invalid URL")
      ...
      $.ajax({
        ...
        type: "POST",
        beforeSend: xhr => {
          xhr.setRequestHeader("url", val)
        },
        success: d => {
          if (d.error) {
            toastr.error(
              "Some error occured on Server Side, Please try again",
              "Unable to Scrape"
            )
          } else if (d.url) {
            toastr.success(
              "Successfully Scraped",
              "Successfully scraped data and made URL"
            )
          }
          $("#inputCSS").removeClass("disabled")
          $("#scrape").removeClass("loading")
